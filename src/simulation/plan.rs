...
pub trait Rectangle: Plan {
    fn vertices(&self) -> Vec<Vertex> {
        (0..self.x_size()).cartesian_product(0..self.y_size())
            .map(|(x, y)| Vertex { x, y, })
            .collect()
    }
}
...
mod test {
    ...
    mod one_three_rectangle {
        ...
        fn test_vertices() {
            ...
            assert!(<OneThreeRectangle as Plan>::vertices(&plan)
                .contains(&Vertex {
                    x: x_size - 1,
                    y: y_size - 1,
                }));
        ...
        fn test_sources() {
            ...
            assert!(plan.sources().contains(&Vertex { x: 0, y: y_size - 2, }));
        ...
        fn test_terminals() {
            ...
            assert_eq!(plan.terminals().len() as u64, (x_size - 2) + (y_size - 2) + (x_size - 2));
            // Top terminal
            assert!(plan.terminals().contains(&Vertex { x: 1, y: y_size - 1, }));
            // Right terminal
            assert!(plan.terminals().contains(&Vertex { x: x_size - 1, y: y_size - 2, }));
            // Bottom terminal
        ...
        fn test_edges() {
            ...
            assert!(plan.edges().contains(&UndirectedEdge {
                first: Vertex { x: x_size - 2, y: 0, },
                second: Vertex { x: x_size - 1, y: 0, },
            }));
            if x_size > 2 && y_size > 1 {
                assert!(plan.edges().contains(&UndirectedEdge {
                    first: Vertex { x: (x_size - 1) / 2, y: (y_size - 1) / 2, },
                    second: Vertex { x: (x_size - 1) / 2 + 1, y: (y_size - 1) / 2, },
                }));
            }
        }
        ...
        fn test_neighbors() {
            ...
            // Edge vertex right
            test!((x_size - 1, 1), [(x_size - 2, 1), (x_size - 1, 2), (x_size - 1, 0)]);
            // Edge vertex top
            test!((1, y_size - 1), [(1, y_size - 2), (0, y_size - 1), (2, y_size - 1)]);
            // Edge vertex bottom
            ...
            test!((x_size - 1, y_size - 1), [(x_size - 2, y_size - 1), (x_size - 1, y_size - 2)]);
    ...
    mod undirected_edges {
        ...
        fn eq() {
            ...
            let c = UndirectedEdge { first, second: third, };
